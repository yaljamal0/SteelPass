...
class Textbox:
    ...
    def draw(self, app):
        ...
        drawLabel(self.text[::-1][:25][::-1], self.x+20, self.y+self.h/2, align='left',
...
def onKeyHold(app, keys):
    if 'backspace' in keys:
        app.textboxes[0].text = app.textboxes[0].text[:-1]
        time.sleep(0.1)

def onKeyPress(app, key):
    if key == 'space':
        app.textboxes[0].text = app.textboxes[0].text[:-2] + ' ' + app.textboxes[0].text[-2:]
    elif key != 'backspace':
        app.textboxes[0].text = app.textboxes[0].text[:-2] + key + app.textboxes[0].text[-2:]
